...
- Compute nodes: perform queries on data. A compute node is partition into slices. Each slice is allocation a portion of memory and disk space, where it processes a portion of workload. 
    - Slices work in parallel, a node can have 2, 4, 16 or 32 slices, depending the resource capacity
...
- Restoring from snapshots `creates a brand new cluster` we cannot restore a cluster from snapshot only a new cluster is possible., we can chose a working AZ to be provisioned into
...
- Amazon Redshift Workload Management (WLM) helps organize and prioritize queries so that short and quick queries don’t get stuck waiting behind long, complex ones. 
- Think of WLM as a traffic management system for queries. It creates different **queues** (like separate lanes on a highway) based on **service classes** (rules that determine how queries are handled). Some of these queues are used internally by Redshift, while others are available for users to control.
- From a user's point of view, a **queue** and a **service class** are basically the same thing because both are used to group queries and apply specific rules to them. So, when you assign a query to a service class, you're essentially deciding which queue it will go into and how it will be prioritized.
